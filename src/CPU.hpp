...
class CPU {
   ...
   public:
    explicit CPU(istream &InputStream)
        : Mem(InputStream), pc(0), pcNext(0), StopCnt(0), BubbleCnt(0), HazardStallFlag(false), StallFlag_IF(false), StallFlag_ID(false), StallFlag_EX(false), IF_ID_DiscardFlag(false), IF(Mem, Pred, pc, pcNext, StopCnt, StallFlag_IF), ID(IF_Buffer_Pre, Reg, StopCnt, StallFlag_ID), EX(ID_Buffer_Pre, Pred, pcNext, StopCnt, StallFlag_EX, IF_ID_DiscardFlag), MEM(EX_Buffer_Pre, Mem, StopCnt), WB(MEM_Buffer_Pre, Reg) {}
